...
btree_node* SEARCH(btree_node *,int number);
void INSERT(int number);
void INORDER (btree_node *);
void DELETE (int number);
void COPY(btree_node *);
   ...
   if (currentptr==NULL)
     { printf ("\n Number %d is not in the tree!!!",number);
      ...
      {	if (currentptr == rootptr)
         	...
         	COPY(previousptr->left);
      	else
         	COPY(previousptr->right);
      }

}

void COPY (btree_node *node)
{
   if(node->right!=NULL)
      node = node->left;
   ...
   else
   {
      ...
      node->number = tmpptr->number;
      
      if (previousptr == node)
      	  previousptr->left = tmpptr->left;
      else
         previousptr->right = tmpptr->left;
